...
const FornecedorCatEditar = () => {
  ...
  useEffect(() => {
    fetch("https://sistemacomercial-fv5g.onrender.com/catfornecedor/" + catcod).then((res) => {
      return res.json();
    }).then((resp) => {
    ...
    if (isValidate()) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          fetch("https://sistemacomercial-fv5g.onrender.com/catfornecedor/" + catcod, {
            method: "PUT",
      ...
      <div className="container" style={{ display: 'flex', margin: '0 230px', marginTop: '-950px' }}>
        ...
        <form action='' onSubmit={editar}>
          ...
          <div className='mb-3'>
            ...
            <input type='text' onKeyUp={MostraNome} placeholder='Entre com o nome:' value={nome} onChange={e => nomechange(e.target.value)} style={{ fontSize: '17px', width: 250, margin: '0 115px', fontWeight: 'bold', color: 'navy' }} className='form-control rounded-0' name='nome' id='nome' />
          </div>
          <div className='mb-3'>
            ...
            <Link to='/fornecedores/categoria/lista' className="btn border rounded-0" style={{ color: 'white', backgroundColor: 'orange', margin: '0 -90px', fontSize: '15px', width: 100 }}>Voltar:</Link>
          </div>
          <ToastContainer />
        </form>
      </div>
...
export default FornecedorCatEditar